  ...
  <div class="space-y-8">
    ...
    <div v-else>
      ...
      <div
        ...
        :class="isAdmin ? 'text-center py-12 bg-muted/30 rounded-2xl border border-dashed border-muted' : 'text-center py-16 bg-muted/30 rounded-2xl border border-dashed border-muted'"
      >
        <UIcon v-if="!isAdmin" name="i-lucide-megaphone" class="text-4xl text-muted mb-4" />
        <h3 v-if="!isAdmin" class="text-lg font-semibold text-highlighted mb-2">No hay anuncios ni votaciones</h3>
        <p class="text-muted" :class="!isAdmin ? 'max-w-sm mx-auto' : ''">
          {{ isAdmin ? 'Aún no hay anuncios o votaciones publicadas para esta asignatura.' : 'Aún no se han publicado avisos para esta asignatura. Vuelve a revisar más tarde.' }}
        </p>
      </div>
      ...
      <div v-else class="space-y-4">
        <template v-for="announcement in announcements" :key="announcement.id">
          <!-- Votación Pública -->
          <PollCard
            v-if="!isAdmin && announcement.type === 'POLL'"
            :announcement="announcement"
            @voted="() => refresh()"
          />

          <!-- Comunicado o Vista Admin de Votación -->
          <UCard
            v-else
            :ui="{
              root: announcement.type === 'POLL' ? 'border-l-4 border-l-indigo-500' : 'border-l-4 border-l-primary',
              header: !isAdmin && announcement.type !== 'POLL' ? 'pb-2 bg-muted/10' : undefined,
              body: !isAdmin && announcement.type !== 'POLL' ? 'pt-4' : undefined,
            }"
          >
            <!-- Public Header para Comunicado -->
            ...
            <!-- Admin Body / Public Body content -->
            <div :class="isAdmin ? 'space-y-4' : ''">
              <!-- Admin Header del item -->
              <div v-if="isAdmin" class="flex items-start justify-between gap-4">
              ...
              <!-- Public Body del comunicado -->
              <p v-else-if="!isAdmin && announcement.type === 'COMMUNICATION'" class="text-default whitespace-pre-wrap leading-relaxed">
                {{ announcement.content }}
              </p>

              <!-- Resultados en tiempo real de la votación (solo para ADMIN POLL) -->
              <div v-if="isAdmin && announcement.type === 'POLL' && announcement.options?.length" class="space-y-3 pt-3 border-t border-muted/30">
